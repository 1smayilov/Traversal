...
namespace DataAccessLayer.EntityFramework
{
    public class EfReservationDal : GenericRepository<Reservation>, IReservationDal
    {
        public List<Reservation> GetListWithReservationByWaitAccepted(int id)
        {
            ...
            {
                return context.Reservations.Include(x => x.Destination).Where(x => x.Status == "Təsdiqləndi" && x.AppUserId == id).ToList();
            }
        }
        ...
        public List<Reservation> GetListWithReservationByWaitPrevious(int id)
        {
            using (var context = new Context())
            {
                return context.Reservations.Include(x => x.Destination).Where(x => x.Status == "Keçmiş Rezervlər" && x.AppUserId == id).ToList();
            }
        }
